...
Page({
  data: {
    ...
    parts: [], partIndex: 0, jointPhase: "parts", jointIndex: 0, jointTests: [], rodIndexes: [],
  ...
  onLoad(options) {
    ...
    if (!stage || stage.renderer !== "canvas" || !game.enterStage(stageId)) { wx.navigateBack(); return; }
  ...
  xrAction({ detail }) {
    ...
    if (kind === "rod") {
      const rods = unique(this.data.rodIndexes.concat(index)); this.setData({ rodIndexes: rods, progress: Math.round(rods.length / 3 * 100) }); audio.playFeedback();
      if (rods.length >= 3) this.finishStage(480); return;
    }
  ...
  setupFallback(stageId) {
    ...
    if (stageId === "joint") {
      ...
      this.setData({fallbackTargets:targets,fallbackItems:items,dragTolerance:54});
