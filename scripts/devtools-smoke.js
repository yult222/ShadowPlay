...
async function tapColor(miniProgram, id) { await callPage(miniProgram, "chooseColor", { currentTarget: { dataset: { id } } }); await wait(80); }
async function screenshot(miniProgram, name) { await miniProgram.screenshot({ path: path.join(OUTPUT, name) }); process.stdout.write(`CAPTURE ${name}\n`); }
...
(async () => {
  ...
  try {
    ...
    for (const group of COLOR_GROUPS) { await tapColor(miniProgram, group.colorId); await tapCraft(miniProgram, group.points[0]); }
